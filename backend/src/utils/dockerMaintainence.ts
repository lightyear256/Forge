...
export const dockerMaintenance = {
  ...
  async fullCleanup() {
    try {
      ...
      await execAsync(
        'docker ps -q --filter "name=exec-" | xargs -r docker kill -s SIGKILL',
        { timeout: 30000 }
      ).catch(() => {});
      await new Promise((resolve) => setTimeout(resolve, 2000));
      await execAsync(
        'docker ps -aq --filter "name=exec-" | xargs -r docker rm -f',
        { timeout: 30000 }
      ).catch(() => {});
      ...
      await execAsync('docker container prune -f --filter "until=1h"', {
        timeout: 30000,
      });
  ...
  async emergencyCleanup() {
    try {
      ...
      await execAsync(
        'docker kill $(docker ps -q --filter "name=exec-") 2>/dev/null || true',
        { timeout: 10000 }
      );
      await new Promise((resolve) => setTimeout(resolve, 2000));
      await execAsync(
        'docker rm -f $(docker ps -aq --filter "name=exec-") 2>/dev/null || true',
        { timeout: 10000 }
      );
  ...
  async logStats() {
    try {
      const { stdout: containerCount } = await execAsync(
        "docker ps -q | wc -l"
      );
      const { stdout: execCount } = await execAsync(
        'docker ps -q --filter "name=exec-" | wc -l'
      );
      const { stdout: imageCount } = await execAsync(
  ...
  async getMemoryUsage() {
    try {
      ...
      const [used, total, percent] = stdout
        .trim()
        .split(" ")
        .map(parseFloat) as any;

      console.log(
        `💾 System Memory: ${used}MB / ${total}MB (${percent.toFixed(1)}%)`
      );
...
export const setupDockerMonitoring = () => {
  ...
  setInterval(async () => {
    ...
    if (memory && memory.percent > 75) {
      console.warn(
        `⚠️ Memory usage at ${memory.percent.toFixed(
          1
        )}% - aggressive cleanup...`
      );
