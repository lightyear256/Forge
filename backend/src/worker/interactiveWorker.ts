...
const forceCleanupContainer = async (
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        ` Cleanup attempt ${attempt}/${maxRetries} for: ${containerName}`,
      );

      try {
        await execAsync(`docker kill -s SIGKILL ${containerName}`, {
          timeout: 10000,
        });
      } catch (e) {}
  ...
  const worker = new Worker(
    ...
    async (job) => {
      ...
      try {
        ...
        if (!hasMemory) {
          throw new Error(
            "Server memory capacity exceeded. Please try again in a moment.",
          );
        }
        ...
        containerName = `exec-${containerId}`;
        ...
        if (language.toLowerCase() === "java") {
          ...
          if (javaClassName) {
            ...
            throw new Error(
              "Could not find public class declaration in Java code",
            );
          }
        } else {
          if (filename) {
        ...
        const setupAndRun = `mkdir -p /app && dd if=/dev/stdin of=/app/${actualFileName} bs=1 count=${byteCount} 2>/dev/null && timeout 32 ${command}`;
        ...
        const dockerCmd = [
          ...
          "sh",
          "-c",
          `${setupAndRun} || exit 124`,
        ];

        console.log(
          ` Spawning: ${containerName} (mem: ${config.memory}, cpu: ${config.cpus})`,
        );
        ...
        const terminateExecution = async (
          ...
          if (docker) {
            try {
              ...
              if (docker.stdout && !docker.stdout.destroyed)
                docker.stdout.destroy();
              if (docker.stderr && !docker.stderr.destroyed)
                docker.stderr.destroy();
  ...
  setInterval(async () => {
    try {
      ...
      for (const name of containers) {
        const isTracked = Array.from(activeProcesses.values()).some(
          (p) => p.containerId === name,
        );
