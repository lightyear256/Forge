...
const TestimonialCard = ({ testimonial, delay }: { testimonial: Testimonial, delay: string }) => (
  <div className={`${delay} flex items-start gap-3 rounded-2xl bg-gradient-to-br from-gray-800/60 to-gray-900/60 backdrop-blur-xl border border-gray-700/40 p-4 w-64 opacity-0 animate-[slideUp_0.6s_ease-out_forwards] hover:scale-105 transition-transform duration-300`}>
    <img src={testimonial.avatarSrc} className="h-10 w-10 object-cover rounded-full ring-2 ring-purple-500/30" alt="avatar" />
    <div className="text-sm leading-snug">
      <p className="flex items-center gap-1 font-semibold text-gray-50">{testimonial.name}</p>
...
export default function Login() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const result = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/user/login`,
        registrationData,
  ...
  return (
    <div className="pt-25 min-h-screen flex flex-col lg:flex-row w-full bg-gradient-to-br from-gray-950 via-black to-gray-900 text-gray-50">
      ...
      <section className="hidden lg:block flex-1 relative p-6">
        <div
          className=" animate-[slideRight_0.8s_ease-out_0.3s_forwards] absolute inset-6 rounded-3xl bg-cover bg-center shadow-2xl"
        >
