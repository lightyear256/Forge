...
import { Play, Save, Terminal, Plus, Trash2, File, Loader2, XCircle, Edit2, Check, X, AlertTriangle, ArrowLeft } from 'lucide-react';
...
export default function CodeEditor() {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/project/delete_file`, {
  ...
  const renameFile = async (oldFilename: string, newFilename: string, id: string) => {
                    ...
                    {renamingFileId === file._id ? (
                      <div className="flex items-center gap-2 p-3" onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          onKeyPress={(e) => {
                            if (e.key === 'Enter') renameFile(file.filename, renameValue, file._id);
                        ...
                        />
                        <button
                          onClick={() => renameFile(file.filename, renameValue, file._id)}
                      ...
                      <div className="p-3 flex items-center justify-between">
                        ...
                        >
                          <div className={`w-1 h-1 rounded-full ${currentFile?._id === file._id ? 'bg-purple-500' : 'bg-slate-700'}`}></div>
                          <span className={`text-sm truncate transition-colors ${
                            currentFile?._id === file._id ? 'text-purple-400' : 'text-slate-400 group-hover:text-slate-300'
                          }`}>
                            {file.filename}
                          </span>
                        </div>
          ...
          <div className="p-3 border-t border-slate-800">
            {isCreatingFile ? (
              ...
              <button
                ...
                title={files.length >= 5 ? "Maximum 5 files allowed" : "Create new file"}
              >
          ...
          <div className="w-full rounded-xl max-w-md bg-black border border-slate-800">
            ...
            <div className="p-6 space-y-4">
              <p className="text-slate-400 text-sm">
                ...
                <span className="text-slate-200 font-medium">"{fileToDelete.name}"</span>?
